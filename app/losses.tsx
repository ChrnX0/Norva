...
import { unidadeEAsNossasDeFora } from '@/data/unidade';
...
function WhatWasLost() {
  ...
  const { data, loading } = useQuery<{ agora: LossRow[]; antes: LossRow[]; dinheiro: boolean }>(async () => {
    ...
    const [agora, antes] = await Promise.all([
      lossesOn(empresaDaqui(), inicio.from, hoje.to, unidadeEAsNossasDeFora()),
      lossesOn(empresaDaqui(), antesInicio.from, antesFim.to, unidadeEAsNossasDeFora()),
    ]);
