...
| **2** | **Quem está operando — a camada 1 do login** | o estudo saiu (`docs/estudo-conta.md`) e mudou a ordem: "login e conta" não é um bloco só. Grade de nomes com PIN, quem está com o aparelho e o responsável do aparelho **não dependem do servidor**, são verificáveis com a foto, e preenchem o `operator_id` que o razão já tem coluna para guardar desde `supabase/migrations/0014_who_was_holding_it.sql:21`. Tratado como bloco único, o trabalho começaria pelo Supabase — a parte que ele mandou adiar. |
| **2b** | **A conta da empresa — a camada 2** | cliente Supabase, sessão, cadastro do dono, convite por código. Espera a sincronia existir de verdade: sem ela, é pagar projeto para uma tela de entrar que não leva a lugar nenhum. A única pergunta de verdade para o dono está no fim do estudo — as outras duas eram preferência de cliente e viraram configuração. |
