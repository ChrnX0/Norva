...
| migrações do servidor | **40** | `ls supabase/migrations \| wc -l` |
...
| **2b** | **A conta da empresa — a camada 2.** ~~O servidor não subiu~~ **— SUBIU em 6 de setembro, decisão (a) do dono.** O projeto `norva` (São Paulo, criado em 1 de setembro) tinha **6** das 39 migrações; as 33 que faltavam foram aplicadas e as dezessete garantias do `db:verify` seguem de pé. Falta o cliente Supabase, sessão, cadastro do dono, convite por código. | cliente Supabase, sessão, cadastro do dono, convite por código. Ela deixou de ser "depois": sem lista de gente não há quem operou, e é esse o motivo novo que a decisão de *"o servidor sobe o mais tarde possível"* não tinha quando foi tomada. |
...
| **3d** | **As quatro peças da capa que NAVEGAM em vez de abrir** | `insumos`, `pedidos`, `expedicao` e `precos` levam para outra tela; as outras nove abrem no lugar. Quem toca não tem como saber qual das duas vai acontecer, e isso é herança, não decisão. |
