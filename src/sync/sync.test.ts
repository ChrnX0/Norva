...
test("a failed write leaves nothing behind, in the data or in the queue", async () => {
  ...
  assert.equal(
    await pendingCount(),
    0,
    "a rolled back write must not leave a queued one",
  );
});
...
test("only what the server confirmed is marked sent", async () => {
  ...
  assert.equal(
    report.remaining,
    before - report.sent,
    "the rest is still queued, to the row",
  );
...
test("a server that throws loses nothing and retries with backoff", async () => {
  ...
  assert.equal(
    report.remaining,
    before,
    "nothing was marked, nothing was lost",
  );
  ...
  assert.deepEqual(
    waits,
    [1_000, 2_000],
    "it waits longer each time, and not after the last",
  );
});
...
test("the drain sweeps what the server took long ago, and never what is still waiting", async () => {
  ...
  const transporte: Transport = {
    push: async (entries): Promise<PushResult> => ({
      acceptedIds: entries
        .filter((e) => primeiros.includes(e.id))
        .map((e) => e.id),
    }),
  };
  ...
  const linhas = await live.getAllAsync<{ id: string }>(
    "SELECT id FROM outbox",
  );
  assert.equal(
    linhas.length,
    aindaEsperando,
    "só sobram as que ainda não subiram",
  );
  assert.ok(
  ...
  assert.equal(
    await pendingCount(),
    aindaEsperando,
    "e nenhuma pendente foi varrida junto",
  );
});
...
test("the sweep spares what went up inside the window", async () => {
  ...
  await drain(
    ...
    { sleep: async () => undefined, maxAttempts: 1 },
  );

  const sobrando = await live.getFirstAsync<{ n: number }>(
    "SELECT COUNT(*) AS n FROM outbox",
  );
  assert.equal(
    sobrando?.n,
    ids.length,
    "quem subiu hoje continua guardado, para poder ser dito",
  );
  ...
  const depois = await live.getFirstAsync<{ n: number }>(
    "SELECT COUNT(*) AS n FROM outbox",
  );
...
test("um aparelho que não sabe de que empresa é não sobe nada — e a fila fica intacta", async () => {
  ...
  assert.ok(
    antes > 0,
    "a fila desta fábrica não pode estar vazia — a asserção seria de graça",
  );
  ...
  assert.equal(
    tentou,
    0,
    "nem uma tentativa: recusar é ANTES de falar com o servidor",
  );
  ...
  assert.equal(
    relatorio.remaining,
    antes,
    "a fila tem de continuar exatamente onde estava",
  );
  assert.equal(
    relatorio.error,
    undefined,
    '"eu não tentei" não é "o servidor recusou"',
  );
});
